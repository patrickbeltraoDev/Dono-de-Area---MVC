...
function conMinhaRV(chapaUser, funcaoUser, otherTable = false) {
    // alert(otherTable)
    showLoad();
    fetch('rvAcoes.php', {
        method: 'POST',
        ...
        .then(data => {
            ...
            if (otherTable == false) {
                ...
                document.getElementById('metaProd').innerHTML = dados.metaProd;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    } else { // SE TIVER ALGUM CARGO DE LIDERANÇA, VAI VERIFICAR SE DENTRO DESSES CARGOS TEM OS CARGOS DO CL/SEF, PARA INCLUIR UM PARAMENTRO NA CHAMADA DA FUNÇÃO QUE VAI ALTERAR A TABELA A SER CONSULTADA

        const cargosOutrasAreas = ['SUPERVISOR CONTROLE LOCAL', 'SUPERVISOR REG GESTAO QUALIDADE', 'COORDENADOR CONTROLE LOCAL', 'COORDENADOR REG GESTAO QUALIDADE', 'COORDENADOR REG SISTEMAS INFORMACAO'];
        const contemCargoOutrasAreas = cargosOutrasAreas.some(cargoOutras => funcaoUsuario.includes(cargoOutras));

        if (contemCargoOutrasAreas) {
            conMinhaRV(chapaUser, funcao, true);
