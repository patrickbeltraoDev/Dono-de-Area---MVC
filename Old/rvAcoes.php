    ...
    if ($_POST['action'] == 'consultarMinhaRV') {
        ...
        try {
            ...
            foreach($resConDadosUser as $row) {
                ...
                $areaApoio = $row['AREA'];
                $areasDeApoio = ['CL', 'SEF', 'QUALIDADE', 'PCP', 'XERIFE'];
                if (in_array($areaApoio, $areasDeApoio)) {

                    if ($row['EIXO'] == 'OPERACIONAL') {
                        $dados['prodOutrasAreas']  = "R$ " . number_format($row['VALOR_FINAL_OS_RET'], 2, ',', '.');
                        $dados['assidOutrasAreas'] = $row['ASSIDUIDADE'] * 100 . "%";
                        $dados['bonusOutrasAreas'] = "R$ " . number_format($row['BONUS'], 2, ',', '.');
                        $dados['rvOutrasAreas']    = "R$ " . number_format($row['RV'], 2, ',', '.');
                        $dados['tag'] = 'OPERACIONAL';

                    } else {
                        $dados['prodOutrasAreas']  = "R$ " . number_format($row['VALOR_FINAL_OS_RET'], 2, ',', '.');
                        $dados['assidOutrasAreas'] = $row['APROVAÇÃO_HE'] * 100 . "%";
                        $dados['bonusOutrasAreas'] = "R$ " . number_format($row['PENALIDADE_HE_VALOR'], 2, ',', '.');
                        $dados['rvOutrasAreas']    = "R$ " . number_format($row['RV'], 2, ',', '.');
                        $dados['tag'] = 'LIDERANÇA';
                    }
                    ...
                    if (isset($row['VALOR_ATINGIDO_EFICACIA'])) {
                        $dadosIndicadores['indEfi'] = "Eficácia: R$" . number_format($row['VALOR_ATINGIDO_EFICACIA'], 2);
                    }
                    if (isset($row['VALOR_ATINGIDO_INFANCIA'])) {
                        $dadosIndicadores['indInf'] = "Infância: R$" . number_format($row['VALOR_ATINGIDO_INFANCIA'], 2);
                    }
                    if (isset($row['VALOR_ATINGIDO_REPETIDA'])) {
                        $dadosIndicadores['indRep'] = "Repetido: R$" . number_format($row['VALOR_ATINGIDO_REPETIDA'], 2);
                    }
                    if (isset($row['VALOR_ATINGIDO_PRODUTIVIDADE'])) {
                        $dadosIndicadores['indPro'] = "Produtividade: R$" . number_format($row['VALOR_ATINGIDO_PRODUTIVIDADE'], 2);
                    }

                    $dados['mediaInd'] = $dadosIndicadores;
                    ...
                    // DADOS PRODUTIVIDADE
                        ...
                        $dados['totalOs']        = "R$ " . number_format($row['VALOR_FINAL_OS'], 2, ',', '.');
                        $dados['totalRet']       = "R$ " . number_format($row['VALOR_FINAL_RET'], 2, ',', '.');
                        $dados['vlrFinalProd']   = "R$ " . number_format($row['VALOR_FINAL_OS'], 2, ',', '.');
                        $dados['vlrFinalRet']    = "R$ " . number_format($row['VALOR_FINAL_RET'], 2, ',', '.');
                    ...
                    // DADOS VALOR FINAL DA RV
                        $dados['valorFinalRV']  = "R$ " . number_format($row['RV'], 2, ',', '.');
        ...
        try {
            ...
            if ($otherTable == true) {// TRUE > SIGNIFICA QUE A CONSULTA VAI SER FEITA NA TABELA DE OUTRAS ÁREAS
                if ($funcao == 'COORDENADOR' || $funcao == 'GESTOR') {
                    ...
                    $origem = "AND ORIGEM = 'APOIO'";
                } else if ($funcao == 'SUPERVISOR') {
                    $tabela = "tbl_telemont_nova_rv_ftth_resultado";
                    $cargo  = 'SUPERVISOR';
                    $cargo_atl  = 'SUPERVISOR_ATUAL';
                    $origem = "AND ORIGEM = 'APOIO'";
                } else if ($funcao == 'ENCARREGADO'){
                    $tabela = "tbl_telemont_nova_rv_ftth_resultado";
                    $cargo  = 'LIDER IMEDIATO';
            ...
            if ($otherTable == true) {
                ...
                $sqlConResTec = "SELECT 
                    ...
                    WHERE 
                        $cargo_atl IN('$nameUser') 
                        AND MES = (SELECT MAX(MES) FROM modulo_rv.tbl_telemont_nova_rv_ftth_resultado)
                        AND (SITUACAO = 'ATIVO' OR (SITUACAO <> 'ATIVO' AND (COM_RV = 'SIM' OR ATIV_ENCERRADAS = 'SIM')))
                ";
